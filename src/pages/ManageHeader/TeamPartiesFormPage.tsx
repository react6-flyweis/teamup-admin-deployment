      ...
      if (category.id === categoryId) {
        ...
        if (subItemId && subItemId !== 'new') {
          newSubItems = newSubItems.map(item =>
            item.id === subItemId ? { ...item, ...subItemData, linkedItemId: targetLinkedItemId, pageType: 'team-parties' } as HeaderSubItem : item
          );
        } else {
          newSubItems.push({
            ...subItemData,
  ...
  if (loading) {
    return (
      <div className="p-6 text-white min-h-screen flex flex-col items-center justify-center">
        ...
        <p className="text-gray-400 text-sm animate-pulse">Loading Team Parties details...</p>
      </div>
    );
  }
